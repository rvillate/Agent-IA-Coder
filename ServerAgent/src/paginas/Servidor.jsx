...
function JobsTooltip({ active, payload, label }) {
  ...
  return <div className="srv-tooltip"><b>{label}</b><span>Jobs del minuto: {jobs}</span><span>Bytes del minuto: {bytes(transfer)}</span></div>
}
...
export function Servidor() {
  ...
  return <div className="srv-dashboard">
    ...
    <div className="srv-metric-grid">
      ...
      <MetricCard icon={<Users/>} title="Procesos activos" value={processes.total || 0} subtitle={`${lastHourJobs} jobs en el rango seleccionado`} badge="Normal" history={h('processes')} color="#2563eb" bars />
    </div>

    <div className="srv-main-grid">
      <section className="srv-card srv-activity">
        <div className="srv-section-head">
          <div><h2>Actividad del servidor</h2><p>{RANGE_LABELS[range]} · histórico por minuto; cada minuto empieza en cero y los anteriores se conservan.</p></div>
        ...
        <div className="srv-legend"><span><i className="blue"/>Jobs por minuto</span><span><i className="purple"/>Bytes por minuto</span></div>
