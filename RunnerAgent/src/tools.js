...
export function classifyJob(job) {
  if (String(job?.type || '').startsWith('browser.')) return job.type === 'browser.open' ? 'heavy' : 'light'
...
export function jobWorkspaceKey(job, guard) {
  const payload = job.payload || {}
  const input = payload.cwd ?? payload.path ?? payload.screenshotPath ?? payload.diffPath ?? payload.file ?? payload.targetPath ?? '.'
  try {
